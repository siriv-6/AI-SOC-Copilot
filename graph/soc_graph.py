...
from agents.threat_hunter import threat_hunter
from agents.mitre_agent import map_to_mitre
from agents.explainability_agent import explain_incident
...
def threat_hunter_node(state: SOCState):
    result = threat_hunter(state["incident"])
...
def mitre_node(state: SOCState):
    mitre = map_to_mitre(state["threat"].attack_type)
...
builder.add_edge(START, "threat_hunter")
builder.add_edge("threat_hunter", "mitre")
builder.add_edge("mitre", "explainability")
